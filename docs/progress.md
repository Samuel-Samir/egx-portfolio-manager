...
- Post-v1.0 feature work, driven directly by the user starting to use the system with their real Thndr portfolio for the first time.
  - **Real Holdings entry**: built `egxpm/record_holding.py` (CLI, reuses `portfolio_engine.apply_action()` — the one canonical weighted-average-cost/quantity implementation), a 16th Copilot tool `record_holding_transaction` (Execute tier, optionally linked to a Recommendation via `Execution.recommendation_id`), and a Dashboard "Add Transaction" form + history table on the Portfolio Holdings Detail page — all three share the same arithmetic. Added `RecommendationRepository.list_all_executions()`. The user then entered their actual holdings live through the Dashboard: ADA (303 units), BMM (18,294 units), PALM (647 units).
  - **Full Arabic (i18n) support**: `egxpm/shared/i18n.py` (`t()`/`is_rtl()`, keyed by the English string itself so untranslated strings degrade to English instead of erroring), a sidebar language toggle in `app.py` with RTL CSS injection, and language-parameterized system prompts (`PromptRegistry.longterm_system_prompt(language=...)` / `swing_system_prompt(...)`) so scheduled Job recommendations can be written in Arabic. The Copilot uses a separate mechanism — no config toggle, just an instruction to reply in whatever language the user writes in.
  - **Major course correction on Dashboard complexity**: the user pushed back hard on the 15-page flat Dashboard ("I don't understand what all these tabs are for"). Realigned via a mockup-driven process (visualize tool) rather than continuing to build blind, converging on: a 4-card home screen (My Portfolio / Long-Term Investments / Swing Trading / Gold Outlook) plus, per a later detailed spec from the user, a company-selector-driven per-company analysis view. This is confirmed direction but not yet built — tracked as task #99.
  - `egxpm/shared/egx_universe.json`: a data-only file (**explicitly no code, no tests, per direct user instruction**) listing all 224 companies currently listed on the EGX, sourced from stockanalysis.com (the same source already used for Fundamentals), for the future company-selector dropdown.
  - **AI Input Context Export** (`egxpm/llm/context_export.py`, config flag `export_ai_context`, default off): `build_raw_context()` reconstructs everything currently persisted about one company directly from the repositories (financial statements + recomputed FinancialMetrics, latest TechnicalSnapshot, news, Score/RiskScore/ConfidenceScore, historical scores, holdings, sector/market summaries, ConfigurationSnapshot); `export_context()` writes both this raw reconstruction and the actual `CuratedContext` sent to the LLM to `exports/{date}/{company_id}/context.json` + `context.md`. Wired into both `run_longterm.py` and `run_swing.py` immediately before their `generate_recommendation()` calls, gated by the config flag. Deliberately kept out of `llm/client.py` — exporting a debug artifact is a Job-level concern, not part of "call the LLM." 5 new tests.
  - **Fixed a real, user-triggered production bug while verifying the above**: once the user's real ADA holding existed (a company with zero price data from any collector — a known, permanent gap), the Home page's `load_allocation()` started raising `InsufficientDataError` and crashing the whole page. Fixed at the Dashboard boundary only — `AllocationCalculator.calculate()` itself still raises for every other caller (correct: a silent gap there would hide a real bug in the Job pipeline) — by excluding held companies with no known price from the arithmetic and surfacing a visible `st.warning` naming them, rather than silently dropping them or crashing. Verified live via `AppTest` against the real database: Home now renders with a warning ("Price data unavailable ... ADA, BMM") instead of an exception.
  - 344 tests passing total (298 from M0-M9 + 46 new across i18n, holdings entry, prompts, and context export).
- **Still open**: the confirmed company-centric Dashboard page (task #99), P/E and P/B ratios (task #100), the Gold Outlook feature (task #94), and a later phase of additional metrics (Beta, ADX/OBV, PEG, EV/EBITDA, dividend fields, Fair Value) explicitly deferred by mutual agreement. Also open: an uncommitted `config.yaml` allocation-targets change (gold folded into `bmm_index`, 0.25→0.30) pending a natural commit point, and an unresolved question about why the working branch is `new_version` rather than `main` (no divergence — one commit ahead — but the user hasn't confirmed whether this was intentional).
